...
export function QuoteDetailView({ id }: QuoteDetailViewProps) {
  ...
  // The Google-authenticated visitor (if any). Their verified email is matched
  // against the devis automatically — no typing needed.
  ...
  // When a visitor is authenticated (e.g. just came back from Google), try
  // their verified email against the devis once, automatically.
        ...
        <Card className="w-full max-w-md p-8 border-none shadow-sm bg-white space-y-5 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Connectez-vous avec l&apos;adresse e-mail à laquelle ce devis vous a été envoyé.
            </p>
          </div>

          {/* Google sign-in — one tap when the client's Google email matches. */}
          <button
            type="button"
            onClick={handleGoogleSignIn}
            disabled={googleLoading}
            className="w-full h-12 flex items-center justify-center gap-3 rounded-xl border border-input bg-white hover:bg-gray-50 transition-colors text-sm font-semibold text-gray-700 disabled:opacity-60"
          >
            {googleLoading ? (
                ...
                {authUser?.email && (
                  <p className="text-xs text-muted-foreground">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        await logout();
                        // Allow a fresh Google attempt with a different account.
